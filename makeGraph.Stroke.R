...
#'
#' @export makeGraph
makeGraph.Stroke <- function( mat, graphdensity = 1,
  communityMethod=NA, getEfficiency = FALSE,
  inverseValuesAsWeights = FALSE, weighted=NULL, binarize=T ) {

  if ( !usePkg("igraph") ) { print("Need igraph package"); return(NULL) }
  if ( !usePkg("psych") ) { print("Need pysch package"); return(NULL) }
  
  if (binarize) {
#     if (!is.null(weighted)) stop('Cannot use a binarized AND weighted graph')
    nEdges = sum(upper.tri(mat))*graphdensity
    thresh = sort( mat[upper.tri(mat)], decreasing=T)[nEdges]
    mat = 1*(mat >= thresh)
    graphdensity=1
  }
  ...
  g1 <- igraph::graph_from_adjacency_matrix( adjacencyMatrix,
    mode = c("undirected"), weighted = weighted, diag = FALSE )
  if (!is.null(weighted) ) {
    edgeWeights <- antsrimpute( igraph::E(g1)$weight )
  } else { edgeWeights = NULL }
  if ( !inverseValuesAsWeights & !is.null(weighted) )
    {
